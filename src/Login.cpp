...
Login::Login(string newSalt = "SaltySalt")
{
    salt = newSalt;
    errorCounter = 0;
...
void Login::sha256(const string str, unsigned char hash[])
{
    SHA256_CTX sha256;
    SHA256_Init(&sha256);
...
string Login::hexHash(unsigned char pass[], unsigned char salt[])
{
    stringstream ss;
...
string Login::returnHash(const string str)
{
